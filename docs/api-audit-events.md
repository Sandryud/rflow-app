...
Матрица описывает целевое покрытие аудита. В текущей реализации AuditEvent подключён ко всем реализованным переходам Release lifecycle: request review, approve, reject, reopen и release.
...
- Для update-событий в `metadata` используется стандартный формат изменения: `changes: { before, after }`.
- Для простых переходов значения можно использовать формат `from` / `to`.
...
| Approve release             | `POST /api/v1/releases/:releaseId/approve`        |         A          | `release.approved`         | `release`     | `{ fromStatus: "IN_REVIEW", toStatus: "APPROVED" }`                                                                                                                           |
| Reject release              | `POST /api/v1/releases/:releaseId/reject`         |         A          | `release.rejected`         | `release`     | `{ fromStatus: "IN_REVIEW", toStatus: "REJECTED" }`                                                                                                                           |
| Reopen release              | `POST /api/v1/releases/:releaseId/reopen`         |         A          | `release.reopened`         | `release`     | `{ fromStatus: "REJECTED", toStatus: "DRAFT", approvalsReset: true }`                                                                                                         |
| Mark release as released    | `POST /api/v1/releases/:releaseId/release`        |         A          | `release.released`         | `release`     | `{ fromStatus: "APPROVED", toStatus: "RELEASED" }`                                                                                                                            |
